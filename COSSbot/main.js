...
try {
	...
	postMessage(JSON.stringify(session));
        ...
        specialPost.open("POST", "http://localhost:4000/data1", false);
        ...
        var tradingMode = 0;
	postMessage("manual mode selected", 0)
	
        while (tradingMode !== 'm' || tradingMode !== 'a') {
	
		if(!WebGUI_connected){
			tradingMode = readlineSync.question('Use COSSbot manually (m), automatic trading (a), or quit (q)?  ');
		}else{
			tradingMode = 'm'; //force manual for WebGUI until automatic is available for production
		}
		...
		if (tradingMode === 'm') {
			...
			while (manualCommand !== 'b' || manualCommand !== 'back'){
				if(WebGUI_connected){
					do{
						var specialPost = new XMLHttpRequest();
						specialPost.open("GET", "http://localhost:8080/manualCmd", false);
						specialPost.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
						specialPost.send()		
					}while(specialPost.responseText == "" || specialPost.responseText == undefined);
				}else{
					manualCommand = readlineSync.question('Please select a command for COSSbot to execute. Type help (h) to see all options:   ');
				}
